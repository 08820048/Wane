...
Wane uses the `0.000` version format for both:
...
```sh
CODESIGN_IDENTITY="Developer ID Application: Name (TEAMID)" ./scripts/build_release.sh 0.000
```
...
```sh
xcrun notarytool submit dist/Wane-0.000.dmg \
  --keychain-profile WaneNotary \
  --wait

xcrun stapler staple dist/Wane-0.000.dmg
xcrun stapler validate dist/Wane-0.000.dmg
```
...
hdiutil attach dist/Wane-0.000.dmg -mountpoint "$MOUNT_DIR" -nobrowse -quiet
...
cp dist/Wane-0.000.dmg /tmp/wane-appcast-input/Wane-0.000.dmg
cat > /tmp/wane-appcast-input/Wane-0.000.md <<'EOF'
# Wane 0.000
...
/path/to/Sparkle/bin/generate_appcast \
  --embed-release-notes \
  --download-url-prefix "https://github.com/08820048/Wane/releases/download/v0.000/" \
  --link "https://github.com/08820048/Wane" \
  -o dist/appcast.xml \
  /tmp/wane-appcast-input
```

Upload these release assets to GitHub Release `v0.000`:
